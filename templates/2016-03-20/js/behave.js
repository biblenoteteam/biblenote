$(document).ready(function(){
    ...
    BookingTop = (function($){
        ...
        init = function(){
            var root = $('#bookingTopWrapper'),
                toggle = root.find('a.toggle'),
                body = $('body'),
                subTabsMenu = root.find('div.tab.menu'),
                subTabs = root.find('div.subtabs');

            toggle.click(function(){
                body.toggleClass('bookingTopOpen');
                return false;
            });    

            subTabsMenu.on('click', 'a', function(){
                var _this = $(this),
                    index = _this.parent().index();

                _this.closest('li').siblings().removeClass('active').end().addClass('active');

                subTabs.each(function(){
                    var _this = $(this);
                    _this.children().removeClass('active').eq(index).addClass('active');
                });
    ...
    // Zmiany z 19 marca
    // Zmieniłem trochę definicje obiektu, żeby był bardziej uniwersalny
    // W poprzedniej paczce /flightsTypeTabs umieściłem tylko dwa taby, nie zwróciłem uwagi, że oneway,
    // który ma tylko jeden kierunek. Więc na głownej też trzeba dodać dolajny tab dla oneway. Dodałem przykład w index.html

    TripType = (function($){
        var init;

        init = function(){
            var root = $('div.whereWeFly'),
